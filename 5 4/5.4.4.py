...
def szyfr_playfair_deszyfruj(szyfrogram,klucz):
    tekst = ""
    nowy_klucz = przeksztalcenie_klucza(klucz)
    nowy_tekst = przeksztalcenie_tekstu_jawnego(szyfrogram)
    ...
    for i in range(0,len(nowy_tekst),2):
        ...
        if index_wiersza_a == index_wiersza_b:
            tekst = tekst + macierz[index_wiersza_a][(index_kolumny_a-1)%5]
            tekst = tekst + macierz[index_wiersza_b][(index_kolumny_b-1)%5]

        elif index_kolumny_a == index_kolumny_b:
            tekst = tekst + macierz[(index_wiersza_a - 1) % 5][index_kolumny_a]
            tekst = tekst + macierz[(index_wiersza_b - 1) % 5][index_kolumny_b]
        else:
            tekst = tekst + macierz[index_wiersza_a][index_kolumny_b]
            tekst = tekst + macierz[index_wiersza_b][index_kolumny_a]
...
szyfrogram = input()
klucz = input()
print(szyfr_playfair_deszyfruj(szyfrogram,klucz))
